...
当多个 Codex、Claude 或 TClaude Session 都在同一个项目目录工作时，终端标题往往无法回答这些问题：
...
- **会话可辨认**：展示模型生成的中文任务短标题与首次 Prompt，区分同一根目录下的多个 Session。
...
标题生成使用标准 OpenAI-compatible `chat/completions` 接口。默认不读取项目 `.env`，也不会在没有配置密钥时向 OpenAI 发送 Prompt。

```bash
export BKANBAN_LLM_BASE_URL="https://api.openai.com/v1"
export BKANBAN_LLM_API_KEY="..."
export BKANBAN_TITLE_MODEL="gpt-4o-mini"
```

也可指向 Ollama、LM Studio、vLLM 或其他兼容服务。本地且无认证的 endpoint 可以不配置 API key。

> 隐私提示：开启标题生成后，首次 Prompt 最多 1200 个字符会发送给你配置的模型服务。
...
- `~/.local/state/notebook-hapi-board/titles.json`：Session ID、Prompt hash、模型名与生成标题

缓存目录和文件权限分别设为 `0700` 和 `0600`。删除方式：
...
- 确认 `BKANBAN_LLM_BASE_URL` 指向 OpenAI-compatible base URL。
- 确认 API key 和 `BKANBAN_TITLE_MODEL` 可用。
- 不配置标题模型不影响标签、状态、详情、跳转和关闭功能。
...
└── titles.py        # OpenAI-compatible 任务标题
```
